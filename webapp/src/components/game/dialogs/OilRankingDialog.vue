<template>
  <v-dialog
    v-model="$store.state.dialogs.oilRanking.show"
    width="520"
    scrollable
  >
            ...
            <v-list-item v-for="(item, index) in items" :key="item.id">
              <v-list-item-content>
                <v-list-item-title class="d-flex justify-space-between">
                  ...
                  <div>
                    <span class="brown--text text--lighten-3">{{
...
export default {
  ...
  computed: {
    isShowing() {
      return this.$store.state.dialogs.oilRanking.show;
    },
  },
  ...
  methods: {
    getRanking() {
      this.http
        .get(`/countries/rankings/INCOMING_OIL`)
        .then((res) => {
    ...
    close() {
      this.$store.state.dialogs.oilRanking.show = false;
    },
  },
